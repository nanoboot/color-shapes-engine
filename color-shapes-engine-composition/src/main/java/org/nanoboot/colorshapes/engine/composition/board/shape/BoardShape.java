...
public class BoardShape extends Shape {
     ...
     * @since 0.0.0
     */
    public BoardShape(final JsonObject shapeJsonObject) {
     ...
     * @since 0.0.0
     */
    public BoardShape(final String id, final int heightIn, final int widthIn) {
    ...
    public final boolean additionalValidate() {
        ...
        boolean heightIsOk = Composition.isInRange(this.getHeight().getValue(), MIN_HEIGHT, MAX_HEIGHT);
        if (!heightIsOk) {
            return false;
        }
        boolean widthIsOk = Composition.isInRange(this.getWidth().getValue(), MIN_WIDTH, MAX_WIDTH);
        if (!widthIsOk) { //NOSONAR
            return false;
        }
        return true;
    }
    ...
    public final boolean setRandomValues() {
        this.getHeight().setValue(getRandom().nextInt(MIN_HEIGHT, MAX_HEIGHT));
        this.getWidth().setValue(getRandom().nextInt(MIN_WIDTH, MAX_WIDTH));
        getHoles().setRandomValues();
        return true;
    }
    @Override
    public final boolean setDefaultValues() {
        this.getHeight().setValue(DEFAULT_HEIGHT);
        this.getWidth().setValue(DEFAULT_WIDTH);
        getHoles().setDefaultValues();
        return true;
    }
}
